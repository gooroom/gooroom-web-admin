...
class DividedAdminManageDialog extends Component {
  ...
  handleValueChangeForIp = (index) => (event) => {
    const { AdminUserProps, AdminUserActions } = this.props;
    const oldIps = AdminUserProps.getIn(["editingItem", "connIps"]).set(
      index,
      event.target.value
    );
  ...
  isSelectedPart = () => {
    ...
    if (
      editObj.get("isClientAdmin") !== undefined &&
      editObj.get("isClientAdmin") === "1"
    ) {
      return true;
    } else if (
      editObj.get("isUserAdmin") !== undefined &&
      editObj.get("isUserAdmin") === "1"
    ) {
      return true;
    } else if (
      editObj.get("isDesktopAdmin") !== undefined &&
      editObj.get("isDesktopAdmin") === "1"
    ) {
      return true;
    } else if (
      editObj.get("isNoticeAdmin") !== undefined &&
      editObj.get("isNoticeAdmin") === "1"
    ) {
      return true;
    } else if (
      editObj.get("isPortableAdmin") !== undefined &&
      editObj.get("isPortableAdmin") === "1"
    ) {
      return true;
    }
    return false;
            ...
            if (confirmValue) {
              ...
              AdminUserActions.createAdminUserData({
                ...
                if (
                  ...
                  AdminUserActions.readAdminUserListPaged(
                    AdminUserProps,
                    compId
                  );
                  this.handleClose();
  ...
  handleTreeNodeCheck = (name, param) => {
    ...
    if (param.isChecked) {
      // add
      newInfoList = AdminUserProps.getIn(["editingItem", name]).push(
        Map({ name: param.name, value: param.value })
      );
    } else {
      // delete
      newInfoList = AdminUserProps.getIn(["editingItem", name]).filter(
        (n) => n.get("value") != param.value
      );
    }
    AdminUserActions.setEditingItemValue({
      name: name,
      value: GRIMTList(newInfoList),
    });
  };
  ...
  handleDeleteConnectIp = (index) => {
    const { AdminUserProps, AdminUserActions } = this.props;
    // delete
    const newIps = AdminUserProps.getIn(["editingItem", "connIps"]).filter(
      (n, i) => i != index
    );
    AdminUserActions.setEditingItemValue({
      name: "connIps",
  ...
  handleDeleteSelectedData = (name, data) => {
    const { AdminUserProps, AdminUserActions } = this.props;
    // delete
    const newInfoList = AdminUserProps.getIn(["editingItem", name]).filter(
      (n) => n.get("value") != data
    );
    AdminUserActions.setEditingItemValue({
      name: name,
    ...
    if (name == "deptInfoList") {
      this.grExtendedTreeListForDept.resetTreeCheckedNode(
        ids ? ids.toJS() : []
      );
  ...
  render() {
    ...
    const editingItem = AdminUserProps.get("editingItem")
      ? AdminUserProps.get("editingItem")
      : null;

    const connectableIpList =
      editingItem && editingItem.get("connIps")
        ? editingItem.get("connIps")
        : GRIMTList([]);
    ...
    // }) : null;
    const selectedDept =
      editingItem && editingItem.get("deptInfoList")
        ? editingItem.get("deptInfoList")
        : null;
    const selectedUser =
      editingItem && editingItem.get("userInfoList")
        ? editingItem.get("userInfoList")
        : null;
    ...
    // }) : null;
    const selectedGroup =
      editingItem && editingItem.get("grpInfoList")
        ? editingItem.get("grpInfoList")
        : null;
    // const selectedClient = (editingItem && editingItem.get('clientInfoList')) ? editingItem.get('clientInfoList') : null;

    const initSecretSaved =
      editingItem && editingItem.get("initSecretSaved")
        ? editingItem.get("initSecretSaved")
        : null;
    ...
    return (
      <div>
        {AdminUserProps.get("dialogOpen") && editingItem && (
          <Dialog
            open={AdminUserProps.get("dialogOpen")}
            scroll="paper"
            fullWidth={true}
            maxWidth="md"
          >
            <ValidatorForm ref="form">
              ...
              <DialogContent>
                <Grid container spacing={0}>
                  <Grid item xs={6} style={{ paddingRight: 20 }}></Grid>
                </Grid>
                <Grid container spacing={0} style={{ marginTop: 10 }}>
                  <Grid item xs={6} style={{ paddingRight: 5 }}>
                    <Grid container spacing={0} style={{ marginTop: 0 }}>
                      <Grid item xs={6} style={{ paddingRight: 5 }}>
                        <TextValidator
                          label={t("lbAdminUserName")}
                          value={
                            editingItem.get("adminNm")
                              ? editingItem.get("adminNm")
                              : ""
                          }
                          name="adminNm"
                          validators={["required"]}
                          errorMessages={[t("msgAdminUserName")]}
                      ...
                      <Grid item xs={6} style={{ paddingRight: 5 }}>
                        <FormControl style={{ width: "100%" }}>
                          ...
                          {window.gpmsain === Constants.SUPER_RULECODE && (
                            ...
                            >
                              <MenuItem value="S" key="SUPER">
                                {t("lbTotalAdmin")}
                              </MenuItem>
                              <MenuItem value="A" key="ADMIN">
                                {t("lbSiteAdmin")}
                              </MenuItem>
                              <MenuItem value="P" key="PART">
                                {t("lbPartAdmin")}
                              </MenuItem>
                            </Select>
                          )}
                      ...
                      <Grid item xs={6} style={{ paddingRight: 5 }}>
                        <TextValidator
                          label={t("lbAdminUserId")}
                          value={
                            editingItem.get("adminId")
                              ? editingItem.get("adminId")
                              : ""
                          }
                          name="adminId"
                      ...
                      <Grid item xs={6} style={{ paddingRight: 5 }}>
                        <TextValidator
                          ...
                          type={
                            editingItem && editingItem.get("showPasswd")
                              ? "text"
                              : "password"
                          }
                          value={
                            editingItem.get("adminPw")
                              ? editingItem.get("adminPw")
                              : ""
                          }
                          name="userPasswd"
                          validators={passwordCheck}
                          errorMessages={[t("msgAdminPassword")]}
                  ...
                  <Grid item xs={6} style={{ paddingLeft: 5 }}>
                    <Card>
                      <CardHeader
                        style={{ padding: 3, backgroundColor: "#a1b1b9" }}
                        ...
                        action={
                          <Button
                            size="small"
                            color="primary"
                            onClick={(event) => this.handleAddConnectIp()}
                            style={{ marginTop: 8 }}
                          >
                            <GRAddIcon />
                          </Button>
                      ...
                      >
                        <List>
                          {connectableIpList &&
                            connectableIpList.map((n, index) => (
                              <ListItem
                                key={index}
                                style={{ padding: "2px 32px 2px 16px" }}
                              >
                                <ListItemIcon style={{ marginRight: 0 }}>
                                  <GRItemIcon fontSize="small" />
                                </ListItemIcon>
                                <ListItemText
                                  primary={
                                    <TextValidator
                                      ...
                                      onChange={this.handleValueChangeForIp(
                                        index
                                      )}
                                      className={classNames(
                                        classes.fullWidth,
                                ...
                                <ListItemSecondaryAction>
                                  <Button
                                    ...
                                    onClick={(event) =>
                                      this.handleDeleteConnectIp(index)
                                    }
                                  >
                                    <DeleteIcon />
                                  </Button>
                  ...
                  <Grid item xs={12}>
                    <DividedAdminManageRuleSelector
                      compId={compId}
                    ...
                    <Grid item xs={12}>
                      <Card>
                        <CardHeader
                          ...
                          title={t("lbResetOtpSecretStatus")}
                        ></CardHeader>
                        <CardContent
                          style={{
                            padding: 0,
                        ...
                        >
                          ...
                          >
                            <Typography>
                              {t("msgResetOtpSecretStatus")}
                              <Checkbox
                                checked={initSecretSaved === "1"}
                                onChange={this.handleStateChange("initSecretSaved")}
                                disabled={false}
                              />
                            </Typography>
                          </FormControl>
...
export default translate("translations")(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(withStyles(GRCommonStyle)(DividedAdminManageDialog))
);
